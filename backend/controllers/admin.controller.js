...
const getStats = async (req, res) => {
  try {
    const [totalRes, successRes, avgRes, infoRes, todayRes] = await Promise.all([
      query(`SELECT COUNT(*) FROM analytics_events WHERE event_type = 'download'`),
      query(`SELECT (COUNT(*) FILTER (WHERE status = 'success')::float / NULLIF(COUNT(*), 0) * 100) as rate FROM analytics_events WHERE event_type = 'download'`),
      query(`SELECT AVG(response_time) as avg FROM analytics_events WHERE status = 'success'`),
      query(`SELECT COUNT(*) FROM analytics_events WHERE event_type = 'info'`),
...
const getCharts = async (req, res) => {
  try {
    ...
    const [overTimeRes, distRes, peakRes] = await Promise.all([
      ...
      `),
      query(`SELECT platform, COUNT(*) as count FROM analytics_events WHERE event_type = 'download' GROUP BY platform ORDER BY count DESC`),
      query(`SELECT EXTRACT(HOUR FROM created_at)::int as hour, COUNT(*) as count FROM analytics_events GROUP BY hour ORDER BY hour ASC`),
    ]);
...
const getFailures = async (req, res) => {
  try {
    const [byType, byPlatform, trend] = await Promise.all([
      query(`SELECT COALESCE(error_type, 'unknown') as error_type, COUNT(*) as count FROM analytics_events WHERE status = 'failure' GROUP BY error_type ORDER BY count DESC LIMIT 10`),
      query(`
        SELECT platform, COUNT(*) as total,
               COUNT(*) FILTER (WHERE status = 'failure') as failures,
               ROUND(COUNT(*) FILTER (WHERE status = 'failure')::numeric / NULLIF(COUNT(*), 0) * 100, 1) as failure_rate
        FROM analytics_events WHERE event_type = 'download' GROUP BY platform ORDER BY failure_rate DESC
      `),
      query(`
        SELECT DATE_TRUNC('day', created_at) as day,
               COUNT(*) FILTER (WHERE status = 'failure') as failures, COUNT(*) as total
        FROM analytics_events WHERE created_at > NOW() - INTERVAL '7 days' GROUP BY day ORDER BY day ASC
      `),
    ]);
...
const getGeo = async (req, res) => {
  try {
    const result = await query(`
      ...
      FROM analytics_events WHERE country IS NOT NULL AND country != '' GROUP BY country ORDER BY count DESC LIMIT 20
    `);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    const [byPlatform, slowRequests] = await Promise.all([
      query(`
        ...
        FROM analytics_events WHERE status = 'success' AND response_time IS NOT NULL GROUP BY platform ORDER BY avg_time DESC
      `),
      query(`SELECT platform, event_type, response_time, created_at FROM analytics_events WHERE response_time > 10000 AND status = 'success' ORDER BY response_time DESC LIMIT 10`),
    ]);
...
const getHealth = async (req, res) => {
  try {
    const result = await query(`
      ...
      FROM analytics_events WHERE created_at > NOW() - INTERVAL '24 hours' GROUP BY platform ORDER BY total_24h DESC
    `);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
const getDevices = async (req, res) => {
  try {
    const result = await query(`
      ...
      FROM analytics_events GROUP BY device_type ORDER BY count DESC
    `);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
const exportCsv = async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT id, event_type, platform, status, response_time, country, error_type, device_type, created_at FROM analytics_events ${where} ORDER BY created_at DESC LIMIT 10000`,
      params
    );
